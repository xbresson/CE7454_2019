...
class ResidualGatedGCNModel(nn.Module):
    ...
    def __init__(self, config, dtypeFloat, dtypeLong):
        ...
        self.voc_edges = config['voc_edges']
        ...
        self.mlp_edges = MLP(self.hidden_dim, self.voc_edges)
        # self.mlp_nodes = MLP(self.hidden_dim, self.voc_nodes_out)
    ...
    def forward(self, x_edges, x_edges_values, x_nodes, x_nodes_coord, y_edges, edge_cw):
        ...
        e = self.edges_values_embedding(x_edges_values.unsqueeze(3))  # B x V x V x H
        x = self.nodes_coord_embedding(x_nodes_coord)  # B x V x H
        # GCN layers
        for layer in range(self.num_layers):
            x, e = self.gcn_layers[layer](x, e)  # B x V x H, B x V x V x H
        # MLP classifier
        y_pred_edges = self.mlp_edges(e)  # B x V x V x voc_edges
        # y_pred_nodes = self.mlp_nodes(x)  # B x V x voc_nodes
